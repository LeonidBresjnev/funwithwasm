            ...
            LazyColumn {
                items(items=this@run.results, key = { it.key}) { item ->
                    Card(
                        ...
                        modifier= Modifier.padding(5.dp).sizeIn(maxWidth = 560.dp, maxHeight = 240.dp)) {
                        Row {
                            ...
                            val features = listOf(
                                ...
                                Pair("Carcinogenesis and mutagenesis and impairment of fertility",item.carcinogenesis_and_mutagenesis_and_impairment_of_fertility),
                            ...
                            LazyColumn(
                                ...
                                items(items=features, key = { it.first }) { iu ->
    ...
    if (showFeature) {
        Dialog(
            ...
            Surface(
                ...
                LazyColumn {
                    items(items = feature) { iu ->
                        if (!htmlContent) Text(iu)
                        else {
